...
| `profiles` | Linked to `auth.users`; email/name/avatar plus degree program, year level, self-declared UPMMC membership and administrator-controlled `upmmc_verified` |
...
Welcome walkthrough (2026-09-13): successful first-time profile/course setup queues a seven-step welcome dialog on the dashboard and acknowledges the sign-in prompt to avoid competing modals. It explains Dashboard, Courses, Theorems, Practice/exam, Bookmarks and Blogs, self-assessed mastery, solution-access boundaries, and the account menu. Next/Back, Skip, Close/Escape and Start exploring are available; the scrollable dialog uses shared Radix focus/keyboard primitives and light/dark styles. It does not navigate through pages or start practice/exams. The account menu's Welcome guide link opens `/dashboard?guide=1` for replay; dismissal removes only that parameter.

Only new setup completion queues automatic display: existing users are not inferred to be new from empty progress or missing browser storage. Pending/dismissed state is per Auth user in localStorage (`matematisko-welcome-v1:<id>`), with a memory fallback if storage fails; it is not synced between devices and contains no profile or study content. Skipping counts as dismissal. A queued guide waits until the dashboard after profile loading completes; the course-study popup is suppressed while pending. No schema, permission or mandatory-onboarding changes. Three targeted tests passed for progression/back, completion/remount, existing-user exclusion, replay, account isolation and storage failure; TypeScript, affected-file lint, production build and diff whitespace checks passed. No authenticated browser visual check or deployment was performed.
...
Course scope represents selected courses for students and administrators; administrators without a saved selection retain unrestricted study access. Admin question/theorem management explicitly loads all courses. `null` means unrestricted and `[]` represents no selected courses. Question/theorem list and detail queries now return empty/null for `[]` without making a request. Omitted scope remains unrestricted. Initial signed-in scope loading blocks study consumers; failed scope recovery and account-switch races still need a separate audit. Course selection is not database authorization: content remains readable regardless of enrollment, subject to the complete-solution membership rule below.

### Study and practice

Complete problem solutions require administrator-verified UPMMC membership, except Math 20, 21, 22, 23 and STAT 101 (exact course code after case/whitespace normalization). Administrators retain full access for content management. `upmmc_member` remains self-declared and grants no access; `upmmc_verified` defaults to false for existing and new profiles and a database trigger prevents students setting it through inserts or updates. App users has a separate verification checkbox and status; `admin_update_profile_verified` saves profile edits and verification atomically with stale-snapshot checks. No existing self-declarations are automatically verified.

Question list/detail reads use the owner-executed, read-only, security-barrier `study_questions` view. It requires a signed-in identity and returns complete prompts/hints/answers, course/topic JSON and either an authorized solution or `Not available`. Direct `questions` RLS hides restricted rows from non-members, including embedded relation reads. The same data path covers dashboard, bookmarks, practice and completed exam review. Stable IDs and learning records are unchanged. Authenticated routes remount when identity or locally observed verification/admin status changes to discard previously loaded data; remote revocation takes effect on new database reads, and already delivered content cannot be recalled. Students should refresh after verification changes.
...
Exam question replacement (2026-09-13): while a paper is active, each question offers Replace question with confirmation. It randomly selects from the currently loaded, user-scoped bank within the outgoing question's course and topic, excluding all IDs already on the paper and deduplicating candidates. This preserves topic coverage, paper length and question position. No spare candidate means a disabled control with an explanation. Replacement clears only the outgoing question's working notes, attempted mark, flag and any rating; other work and the absolute start/deadline remain intact. Answers stay hidden. Confirmation rechecks the actual deadline, blocking late clicks before the next timer tick, and replacements are unavailable after finishing. Replacements can be repeated; previously removed questions may be drawn again, since there is no replacement-history exclusion. The existing per-user sessionStorage shape persists the new IDs and remaining work without storing question content or changing database progress. Existing papers and retry papers support the same behavior. No schema or permission changes.

Validation: 15 targeted exam tests passed, including cancellation, same-course/topic candidates, duplicate prevention, unavailable replacements, retained timer/other work, persistence/remount and deadline races. TypeScript, affected-file lint and production build ran through installed package executables because npm is unavailable. No deployment or authenticated browser visual check was performed.

### Content rendering and design

Markdown source search (2026-09-16): all admin question/theorem MathEditor fields and the note/blog content editors use a shared FindTextarea. Find or Ctrl/Cmd+F within an editor opens a local, case-insensitive literal search, including LaTeX characters. Match counts, no-results feedback, wrapping Previous/Next, Enter/Shift+Enter in the search field, and Escape are available. Navigation selects and scrolls to the source match for editing; selected source text prefills Find. Ctrl/Cmd+Enter navigates from the source while Find is open. Searches stay independent per field, update against current text, and do not change content or save it. Contribution forms inherit Find through MathEditor. Existing previews, snippets, stable IDs and persistence remain unchanged; no migration or permission changes. Validation: seven targeted FindTextarea/MathEditor tests, TypeScript, affected-file ESLint, production build and diff whitespace checks passed. Browser visual/scroll verification and deployment were not performed.

Course-note environments (2026-09-13): note bodies and the notes-admin preview opt into parsed-Markdown environment panels in MathRenderer. Headings beginning Theorem, Lemma, Corollary or Proposition use maroon panels; Definition uses teal; Proof headings and paragraphs beginning **Proof.** use neutral panels. Labels and original heading levels remain visible, preserving contents navigation and numbering. Panels end at the next heading, environment or horizontal rule; proofs also end after a block ending in ∎ or □. Nested list/blockquote content remains parsed Markdown. Use a heading or horizontal rule to explicitly end an environment when returning to ordinary prose. Existing stored notes and applied migrations are unchanged.

The note detail page supplies the existing user-scoped theorem bank. A unique normalized theorem-name match within the note's course adds “Practice this theorem”; normalization ignores case, accents, punctuation, possessives and the word “theorem”, without fuzzy matching. Ambiguous/unmatched names receive no automatic link. Authors can select an existing same-course stable identity with `### Theorem: [Display name](/theorems/THEOREM-ID)`. The notes editor documents this syntax and previews panel styles; automatic links resolve on the student note page. No new content records, schema, permissions or progress behavior are introduced.

`/theorems/flashcards?theorem=ID` opens a single-card session from the authorized loaded bank, optionally intersected with the existing course filter. Missing/unavailable IDs show the empty state instead of substituting unrelated cards. Deck state resets when the selection changes. Existing rating persistence is retained. Local validation: 19 targeted rendering/navigation/flashcard tests passed, including representative display equations through MathRenderer, code preservation, exact links and missing/ambiguous references; affected-file lint, TypeScript and production build passed using installed package executables (npm unavailable, dependency access required for tests/lint/typecheck). No authenticated browser visual check or deployment was performed.

Use `MathRenderer` for Markdown/LaTeX: `$...$` inline; display math requires opening and closing `$$` on their own lines with the equation between them. Root `AGENTS.md` contains mandatory authoring examples and checks for OpenCode and other contributors, including JSON/SQL escaping. The renderer also tolerates legacy same-line double-dollar math: `remarkDisplayMath` promotes parsed double-dollar inlineMath nodes to KaTeX display mode without rewriting stored source, code examples, or escaped dollars. It decodes Unicode escapes and uses remark-math, remark-gfm, and rehype-katex with non-throwing math errors. Raw HTML is not enabled. Preserve readable long equations, inline rendering inside titles, preview rendering, and light/dark readability.

Math overflow (updated 2026-09-12): `MathRenderer` measures visible formula width and uses ResizeObserver to recheck after layout/font changes. Only formulas exceeding their container width by more than one pixel receive horizontal scrolling; short formulas retain visible overflow so glyph overhang does not create distracting scrollbars. Display formulas are explicitly centered in a surface at least as wide as the container, growing to the formula's natural width when necessary so the start of oversized formulas remains reachable by horizontal scrolling. Inline and display formulas stay unbroken. Hidden accessible MathML is preserved.
...
Duplicate-report deployment repair (2026-09-13): a live report of repeated same-category submissions exposed that `20260913000001_question_report_duplicates.sql` had not been applied. Verified the app environment matches the linked database and a push dry run contained only that migration. Applied it successfully, preserving all existing reports, then confirmed the remote migration set is up to date. Existing frontend report submission already displays RPC error messages, so duplicate rejection does not require a frontend redeployment. The CLI emitted a non-blocking local Docker catalog-cache warning. No live test reports were submitted or deleted; behavioral rejection was tested previously with synthetic data in isolated PGlite.

Question report deduplication (2026-09-13): one open report per question and issue category across all reporters. Category is the reason; optional description differences do not bypass the rule, including for Other. Different categories remain allowed. Resolving or deleting the last open report frees that reason for submission again. Theorem reporting is unchanged. Duplicate submissions return “A report has already been sent for this question with the same reason. Please wait for it to be addressed.” The dialog announces the error and retains entered details.

Append-only migration `20260913000001_question_report_duplicates.sql` adds a private, RLS-enabled reservation/count table and a security-definer trigger covering inserts, updates/reopens and deletes through both RPCs and direct writes. Its primary key arbitrates competing submissions atomically without exposing other reporters' identities or descriptions. Existing duplicate reports are preserved and counted; all must leave open status before that reason becomes available again. No old migrations or report records are rewritten. Apply this migration before relying on hosted enforcement; frontend deployment adds the explanatory copy and accessible error announcement.

Validation: `scripts/check-report-duplicates.mjs` passed isolated PGlite upgrade and clean replay of 82 migrations with synthetic records, including existing duplicates, cross-user rejection, different categories, direct inserts, resolution/reopening/deletion, private-table access and anonymous denial. The primary-key concurrency mechanism was inspected; simultaneous independent database connections were not exercised by PGlite. The report-dialog test, TypeScript, affected-file lint, production build and diff whitespace checks passed. No production migration, user data change or frontend deployment was performed.
...
Reports now expand an editor inside the report card, preserving the description while fetching the affected question/theorem directly by its stable ID. Updated 2026-09-13: a successful question/theorem save resolves the associated report through the existing admin RPC, refreshes the report list, and closes the editor. Failed content saves retain the form; if resolution fails after content saves, the editor shows that partial success and offers a resolution-only retry. Missing content and fetch failures have a retry state. No schema or permission changes.

Updated 2026-09-15: after a successful save and report resolution, the question/theorem report editor closes and scrolls the page to the top immediately. Resolution-only retries do the same on success; failed saves/resolution and cancellation do not trigger this scroll.

Report deletion (2026-09-15): question/theorem reports use the shared Radix dialog instead of browser confirmation. A labeled Delete action opens course/status/category, math-rendered title and report description, with Keep report initially focused and a destructive confirmation. Deletion uses the existing admin-only RPC, blocks duplicate requests and dismissal while pending, retains errors for retry, and removes the confirmed row/count locally with a success message. Focus returns to the originating control on cancellation or to the Reports heading after deletion. Only the report is removed; source content is retained. No schema or permission changes. Validation: nine targeted report tests passed, including safe cancellation/focus restoration, both report types, pending dismissal protection, retry, row removal and counts. TypeScript, affected-file ESLint, production build and diff whitespace checks passed. No hosted-browser visual check or deployment was performed.
...
### 2026-09-12 — Verified UPMMC solution access

Migration `20260912000008_verified_member_solutions.sql` adds administrator verification, protected direct question reads and the redacted study view described above. Existing membership declarations do not become verified automatically. Deployment must apply this migration before releasing the frontend; older frontends querying raw questions will omit restricted-course questions after the database change, so coordinate the rollout and refresh clients. This implementation remains local; no live membership records were verified and no migration or frontend deployment was performed.

Validation: isolated PGlite upgrade and clean replay of 81 migrations passed, including all five course exceptions, near-match course denial, self-declared membership denial, anonymous/direct-table boundaries, administrator access, verification, revocation, stale edits and cross-user isolation. The targeted UI/data suite passed 32 tests, followed by all 8 exam tests including the newly added redacted-review case (33 distinct targeted tests). TypeScript, affected-file lint, production build and diff whitespace checks passed using installed executables with dependency access. Browser testing of authenticated hosted sessions was not performed. No mathematical content was changed.


### 2026-09-15 — Profile course selections also personalize administrator study pages

The shared course provider previously bypassed saved selections for every administrator, so profile edits did not affect their Courses, Dashboard, or other study pages. Administrators now use their selected courses for study, just like students. Administrators with no saved selections retain an unrestricted fallback for existing accounts. Admin question/theorem management explicitly requests all courses through the shared data hooks; the bypass is enabled only for administrators. Database authorization, full-solution access, stable content IDs, bookmarks and progress are unchanged.
...
Deployment update (2026-09-15): restored the local link to the authenticated MatematIsko Supabase project and confirmed that `20260915000001_content_contributions.sql` was the only pending migration. Applied it successfully at the user's request; a subsequent dry run reported the remote database up to date with no pending migrations. The CLI warning about its Docker-dependent local catalog cache did not prevent application. No production submissions or moderation actions were used for verification. Frontend deployment remains separate and was not performed.

### 2026-09-16 — Study capture deterrents

`AppShell` mounts `StudyPrivacy` for signed-in users after authentication loading. Following user feedback about visual clutter, watermarks are now confined to question bodies/previews and revealed hints, answers and solutions in question details, cards, practice and exam review. `QuestionWatermark` receives the current authenticated email through a shell context and displays a faint repeated email pattern plus a small personal-study-copy attribution. The email replaces the shortened account ID at the user's explicit request to identify shared copies. No timestamp, new storage or telemetry is used. XML escaping protects SVG text embedding; missing emails omit the watermark. The overlay is pointer-transparent and screen-reader-hidden, with readable attribution below the content. Navigation, notes, theorem content and other surrounding UI have no watermark. Print backgrounds request watermark preservation, but browser print settings can override them.

All signed-in AppShell pages receive a solid privacy cover when the document is hidden or the window loses focus, including initial unfocused loads. Returning to a visible, focused window restores the view. A solid cover was chosen over blur to avoid legible text beneath it. Content remains mounted, preserving working notes, reveal state and navigation; exam deadlines continue running. Public signed-out pages are unaffected. Watermarks are excluded from profile, administration, contribution and blog pages. Focus-based coverage is a visual privacy measure, not an access boundary, screenshot detector, or proctoring feature; browser controls/devtools can bypass it, and captures without focus loss remain possible.

Existing solution authorization is unchanged: verified-member/admin/course-exception checks in migration `20260912000008_verified_member_solutions.sql` redact `study_questions` and protect raw question reads before restricted solutions reach unauthorized clients. Inspected current `db.ts` study reads and subsequent migration references; no new permission or SQL changes were needed. Already delivered authorized content cannot be recalled. No mathematical content or stable IDs changed.

Validation: `npm test` passed all 292 tests in 45 files with configured coverage thresholds, including focus/visibility restoration, watermark identity/time updates, exclusions via the watermark prop, and portal/timer cleanup. `npm run typecheck`, affected-file ESLint, `npm run build` and diff whitespace checks passed. No authenticated browser visual check, live database verification or deployment was performed. Frontend deployment is required to enable these local changes.


Email watermark refinement (2026-09-16): replaced the viewport overlay with content-local watermarks after feedback that the broad pattern cluttered the website. Kept the independent focus privacy cover and existing database authorization unchanged. No math source or stable content IDs changed.

Refinement validation: all 293 tests in 46 files passed with coverage thresholds, including content-local placement, email switching/removal and XML escaping. TypeScript, affected-file ESLint, production build and diff whitespace checks passed. No authenticated browser visual inspection or deployment was performed.


Blog illustration hosting (2026-09-16): published the Navier-Stokes undergraduate explainer through the existing database-backed blog workflow at `/blogs/from-mantle-flow-to-navier-stokes`, with the author inherited from the existing cryptography post. Created the user-approved public Storage bucket `navier-stokes-blog-20260916` for its three PNG illustrations, limited to PNG uploads of at most 5 MiB per object. No public upload policies were added. Objects in this bucket are publicly readable, including any future uploads; reserve it for this article's public illustrations. Source and website-ready Markdown are in `blogs/navier-stokes-undergraduate/`. This was a content publication, not a schema migration or frontend deployment. Verified the exact published Markdown through the anonymous public view and checked that all three public image downloads match the source file hashes.

### Math 146 Unit II expansion (2026-09-17)

Prepared `notes/math146-unit2-differential-geometry.md` from the supplied Unit II PDF and Typst, with 31 extracted original PNG illustrations in `notes/assets/math146-unit2/`. Preserves the author/professor/template credits, the dated lecture sequence, exercises and supplied proof arguments. Consolidates repeated recaps and documents corrections to non-unit-speed Frenet formulas, helix curvature, differential target space, local IFT hypotheses, sphere radius expressions and parameter-curve evaluation. The two Meeting 19 entries retain their dates; the incomplete Meeting 21 and unspecified homeomorphism exercise are explicitly identified.

Append-only migration `20260917000001_expand_math146_unit2_notes.sql` expands existing note `e6c40a16-e845-41ed-83c2-dabd361cb794` in place, preserving course, title, order and URL. Earlier applied migrations are not edited by this change. Its image URLs target a dedicated `math146-unit2-notes-20260916` public PNG-only Storage bucket, with a 5 MiB object limit and no public write policies added. After explicit user approval, created the bucket and uploaded all 31 illustrations, then applied the migration to the linked database. The dry run contained only this migration. Verified the live content MD5 matches the prepared Markdown exactly and all 31 public PNG downloads match the local SHA-256 hashes. The original note title, ordering, course and URL are preserved. This content update requires no frontend deployment. The CLI reported a non-blocking Docker catalog-cache warning after successful application. Reserve the dedicated public bucket for these illustrations; any objects uploaded there are publicly readable.

Validation: all 60 display equations and 294 inline expressions parsed with KaTeX; all 31 local image references resolve. A temporary full-note check through the app's MathRenderer and four existing note-environment tests passed (5 tests total), verifying rendered equation/image counts and no KaTeX errors. The temporary check was removed after execution. Representative PDF pages were visually inspected, including the spherical angle convention, helix calculation, non-unit-speed recap, IFT example and incomplete final page. No authenticated hosted-browser check has been performed.

### Math 146 Unit I expansion (2026-09-17)

Prepared `notes/math146-unit1-differential-geometry.md` from the supplied 88-page Unit I PDF and Typst, retaining 15 dated sections (Meetings 1-14 and 9.5), author/professor/template credits, exercises and supplied proof arguments. All 28 embedded illustrations are extracted to `notes/assets/math146-unit1/`. Repeated recaps are consolidated. Documented corrections include boundary-point membership, the open domain in the smooth-composition proof, pointwise product-rule evaluations, the two-form basis, div/curl composition, the 3-curve's derivative, speed's square root, endpoint domains, tensor/wedge distinction and missing connection coefficients. The undefined field B in exercise star, missing illustration placeholders and uncontextualized trailing expression are identified rather than reconstructed.

Append-only migration `20260917000002_expand_math146_unit1_notes.sql` replaces the existing Unit I note body while preserving its stable ID `d6c40a16-e845-41ed-83c2-dabd361cb794`, title, course and ordering. Images target the dedicated public PNG-only bucket `math146-unit1-notes-20260917`, limited to 5 MiB per object, with no public upload policies added. The user's request to repeat the approved Unit II workflow includes publishing this note and its illustrations. Publication verification is pending.

Validation: all 108 display equations and 575 inline expressions parsed with KaTeX; decoded migration content matches the authored Markdown with hosted URLs. All 28 local image references and upload sizes passed checks. A temporary full-note MathRenderer test plus four existing note-environment tests passed, confirming 108 rendered display equations, 28 images, all 15 meeting sections and no KaTeX errors. The temporary test was removed. Representative PDF pages and the embedded maps connecting vector fields and forms were visually inspected. No authenticated hosted-browser visual check has been performed.
