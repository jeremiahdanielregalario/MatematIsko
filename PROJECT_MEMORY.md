...
`AppShell` now mounts `StudyPrivacy` for signed-in users after authentication loading. Dashboard, courses/notes, questions, theorems/flashcards, practice/exam and bookmarks show a repeated viewport watermark containing the product name, first 13 sanitized characters of the Auth user UUID and a UTC minute timestamp refreshed every minute. No email, name, token, new storage or telemetry is used. The pointer-transparent, screen-reader-hidden watermark is rendered through a body portal above dialogs and route animations, preserving math rendering and interactions. Print backgrounds request watermark preservation, but browser print settings can override them.
